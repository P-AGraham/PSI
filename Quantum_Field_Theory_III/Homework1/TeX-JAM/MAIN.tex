...
\begin{enumerate}
  ...
  \end{align*}
  The previous implication suggests the power-law ansatz $F_3 = a p^b$ depending on constants $a, b$. Substituting this ansatz, we get $a p^b = a p^b \lambda^{-b-2} \implies b=-2$. The final form for the correlator is 
  \begin{align*}
    \langle j_{\mu}^{V}(-p) \ j_{\nu}^{V}(p) \rangle = \frac{a}{p^2}\left(-p^2\eta_{\mu\nu} + p_\mu p_\nu\right).
  \end{align*} 
  \item[(b)] We consider the axial current defined by $j^A_\mu = \epsilon_{\mu\nu} j^{V, \nu}$. In $D=2$, the non zero components of the levi-civita tensor are $\epsilon_{01} = -\epsilon_{10} = 1$. To determine if the axial current is classically conserved, we calculate its divergence as follows 
  \begin{align*}
