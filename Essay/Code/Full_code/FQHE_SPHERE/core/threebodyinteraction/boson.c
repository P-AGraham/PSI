...
void cal_lambda(double *lambda, double *qx, double *qy, int bound, int level, size_t ax, double g11, double g12, double g22);
...
void A_Three_Body_0(complex double *A_list, complex double *A_list_0, double *qx, double *qy, \
					int bound, int N_phi, int level, int num_thread, double g11, double g12, double g22);
					...
					int bound, int N_phi, int level, int num_thread, double g11, double g12, double g22)
{
	...
	for(size_t ax=0;ax<d;++ax)
		cal_lambda(lambda+ax*d*d*d, qx, qy, bound, level, ax, g11, g12, g22);
...
void cal_lambda(double *lambda, double *qx, double *qy, int bound, int level, size_t ax, double g11, double g12, double g22)
{
	...
	d = 2*bound + 1;
	stride = d * d;
	double qax, qay, qbx, qby, q1, q2, q12;
	qax = qx[ax];
	...
	for(size_t bx=0;bx<d;++bx)
	{
		...
		for(size_t ay=0;ay<d;++ay)
		{
			...
			for(size_t by=0;by<d;++by)
			{
				...
				lambda[bx*stride+ay*d+by] =exp(-(q1+q2+q12)/4.0);
			}
		}
	}
	return;
...
void A_Three_Body_0_summary(complex double *A_list, complex double *A_list_0, int N_phi)
{
	...
	for(j4=0;j4<N_phi;++j4)
	{
		for(j5=0;j5<N_phi;++j5)
		{
			J45 = j4 + j5;
			if(J45>=N_phi)
				J45 -= N_phi;
			for(j6=0;j6<N_phi;++j6)
			{
				J456 = J45 + j6;
				if(J456>=N_phi)
					J456 -= N_phi;
				for(j3=0;j3<N_phi;++j3)
				{
					for(j2=0;j2<N_phi;++j2)
					{
						J32 = j2 + j3;
						if(J32>=N_phi)
							J32 -= N_phi;
						for(j1=0;j1<N_phi;++j1)
						{
							J123 = J32 + j1;
							if(J123>=N_phi)
								J123 -= N_phi;
