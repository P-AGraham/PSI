...
extern "C" size_t L2Z2Count(MKL_INT *indptr, int num_thread)
{
    ...
    for(size_t i=0;i<A.dim;++i)
        indptr[i+1] = L2Z2Count_thread(A.basis, A.hashT, A.Z2_phase, A.Z_2, A.dim, A.N_o, A.N_e, i);
...
extern "C" size_t L2Z2Generate(MKL_INT *indptr, MKL_INT *indices, Data_dtype *data, int num_thread)
{
    ...
    for(size_t i=0;i<A.dim;++i)
    {
        auto count = L2Z2Generate_thread(indptr, indices, data, A.basis, A.hashT, A.Z2_phase, A.Z_2, A.dim, A.N_o, A.N_e, i);
                ...
                if( v!=u && v!=u+1 &&LminusQ(bra_i, v, N_o) )
                {
                    ...
                    if(ind!=ULIMAX)
                    {
                        ...
                        if( n_find==2 && Z_2*Z2_phase[ind]<0 )
                            data_thread[count] *= -1;
                        data_thread[count] *= std::sqrt( std::abs((double)Z2_phase[row]/Z2_phase[ind]) );
